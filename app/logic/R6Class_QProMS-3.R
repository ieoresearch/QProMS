  ...
  public = list(
    ...
    preprocessing = function() {
      ...
      if (self$input_type == "External") {
        ...
        if ("Stripped.Sequence" %in% colnames(initial_table)) {
          ...
          initial_table <- initial_table %>%
            mutate(peptide_names = .data[["Modified.Sequence"]])
        } else {
          initial_table <- initial_table %>%
        ...
        if (self$analysis_level == "peptide" && any(!is.na(initial_table$peptide_names))) {
          initial_table <- initial_table %>%
            mutate(feature_names = peptide_names)
          self$feature_id_col <- "peptide_names"
        } else {
          initial_table <- initial_table %>%
            mutate(feature_names = gene_names)
      ...
      data <- initial_table %>%
        select(all_of(c("gene_names", "feature_names", intensity_cols, intersect(c("peptide_names"), colnames(initial_table),intersect(required_columns, colnames(initial_table)))))) %>% 
    ...
    imputation = function(imp_methods, shift, scale, unique_visual = FALSE) {
      ...
      # -----------------------------
        ...
        wide <- data %>%
          ...
          group_by(feature_names, label) %>%
          summarise(
            intensity = mean(intensity, na.rm = TRUE),
            .groups = "drop"
          ) %>%
          mutate(intensity = if_else(is.nan(intensity), NA_real_, intensity)) %>%
          pivot_wider(
            names_from = label,
        ...
        rownames(wide_df) <- wide_df$feature_names
        
        wide_mat <- as.matrix(wide_df[, setdiff(names(wide_df), "feature_names")])
        ...
        self$imputed_data <- data %>%
          ...
          select(-intensity) %>%
          left_join(imputed_long, by = c("feature_names", "label")) %>%
    ...
    plot_protein_counts = function() {
      ...
      p <- self$filtered_data %>%
        ...
        e_y_axis(
          name = paste(self$get_feature_label(), " counts"),
          nameLocation = "center",
